from sort.sort_all import *
import cv2
import csv
from glob import glob
import os

mot_tracker = Sort(max_age=20, iou_threshold=0.1, min_hits=5)
colors = {}
...
def show_matches(boxes1, boxes_old, im1):
    ...
    for i in range(len(boxes_old)):
        ...
        cv2.rectangle(img=im1, pt1=(int(pts1[0]), int(pts1[1])),
                      pt2=(int(pts1[2]), int(pts1[3])),
                      thickness=15, color=(255, 255, 255))
        im1 = cv2.putText(im1, str(idx_1), (int(pts1[0]), int(pts1[1])), cv2.FONT_HERSHEY_SIMPLEX,
                          fontScale=2, color=(255, 255, 255), thickness=5)
...
for i in range(len(images)):
    ...
    if(len(boxes)==0):
        continue
    track_bbs_ids , track_bbs_ids_old= mot_tracker.update(np.array(boxes))
    show_matches(track_bbs_ids, track_bbs_ids_old, im)
